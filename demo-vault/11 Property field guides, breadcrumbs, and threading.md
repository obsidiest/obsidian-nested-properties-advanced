...
4. Tab into the breadcrumb to navigate with the arrow, Home, and End keys. Click a row once (or activate its button with Enter/Space) to place the caret at the key's end in both Live Preview and Source. Move the pointer away, let the popover close, and type to check that the editor retains the caret.
