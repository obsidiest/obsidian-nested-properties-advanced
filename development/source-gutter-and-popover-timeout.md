...
The first desktop run passed all new gutter and navigation cases on both platforms, but the numeric-entry test did not reach the expected value. A follow-up focus assertion established that its native click never reached the control. Inspection of the actual Obsidian 1.13.7 application showed that `settingsPopoutWindow` defaults to true: Settings owns a separate window, while the test was sending events to the note's window. The test now addresses the number input's own window and runs in both Settings layouts. It still requires decimal entry, Enter, an unchanged input element, and persistence after plugin reload. The initial Linux run also failed one Escape focus precondition; the unchanged history assertions passed in the diagnostic follow-up.
