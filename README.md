...
- **Navigate a field's ancestry** from a clickable, scrollable, keyboard-navigable hover breadcrumb in Live Preview, Source, or Reading mode, with full-field, full-key, and icon activation scopes plus optional full-name wrapping. In Source mode, the fold-gutter area beside every property activates the breadcrumb, including leaves without an expansion toggle.
- **Set the hover breadcrumb timeout** globally or individually for Live Preview, Source, and Reading mode. Numerical inputs accept decimal seconds; the default is 1 second. Enabled individual controls override the global value. The popover remains available while crossing the gap to its navigation buttons.
