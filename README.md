...
- **Navigate a field's ancestry** from a clickable, scrollable, keyboard-navigable hover breadcrumb in Live Preview, Source, or Reading mode, with full-field, full-key, and icon activation scopes plus optional full-name wrapping. In Source mode, the fold-gutter area beside every property activates the breadcrumb, including leaves without an expansion toggle. A single click places the caret at the key's end in both Live Preview and Source. Hover previews and popover dismissal leave that caret in the editor.
